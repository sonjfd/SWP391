                            ...
                            <c:forEach var="medicine" items="${medicineList}" varStatus="counter">
                                <tr data-status="${medicine.status}" >
                                    ...
                                    <td>
                                        ...
                                        <form method="post" action="${pageContext.request.contextPath}/admin-toggle-medicine-status" style="display:inline;">
                                            <input type="hidden" name="id" value="${medicine.id}" />
                                            <input type="hidden" name="status" value="${medicine.status == 1 ? 0 : 1}" />
                                            <button type="submit"
                                                    class="action-btn ${medicine.status == 1 ? 'delete-btn' : 'edit-btn'}"
                                                    onclick="return confirm('Bạn có chắc muốn ${medicine.status == 1 ? 'ẩn' : 'hiện'} thuốc này không?')">
                                                ${medicine.status == 1 ? 'Ẩn' : 'Hiện'}
                                            </button>
