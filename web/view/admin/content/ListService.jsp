                        ...
                        <tbody id="serviceTbody">
                            <c:forEach items="${listService}" var="service" varStatus="counter">
                                <tr data-price="${service.price}" data-status="${service.status}">
                                    ...
                                    <td>
                                        <form method="post" action="${pageContext.request.contextPath}/admin-toggle-service-status" style="display:inline;">
                                            <input type="hidden" name="id" value="${service.id}" />
                                            <input type="hidden" name="status" value="${service.status == 1 ? 0 : 1}" />
                                            <button type="submit"
                                                    class="action-btn ${service.status == 1 ? 'delete-btn' : 'edit-btn'}"
                                                    onclick="return confirm('Bạn có chắc muốn ${service.status == 1 ? 'ẩn' : 'hiện'} dịch vụ này không?')">
                                                ${service.status == 1 ? 'Ẩn' : 'Hiện'}
                                            </button>
                                        </form>
