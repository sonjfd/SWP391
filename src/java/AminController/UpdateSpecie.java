 ...
 * @author FPT
 */
@WebServlet(name="UpdateSpecie", urlPatterns={"/updatespecie"})
public class UpdateSpecie extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet UpdateSpecie</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet UpdateSpecie at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            Specie specie = specieDAO.getSpecieById(id);
            if (specie != null) {
            ...
            List<Specie> specieList = specieDAO.getAllSpecies();
            request.setAttribute("specieList", specieList);
            request.setAttribute("message", "Failed to load update specie form.");
            request.getRequestDispatcher("view/admin/content/ListSpecie.jsp").forward(request, response);
        }
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            specie.setId(Integer.parseInt(request.getParameter("id")));
            specie.setName(request.getParameter("name"));

            boolean success = specieDAO.updateSpecie(specie);

            List<Specie> specieList = specieDAO.getAllSpecies();
            request.setAttribute("specieList", specieList);
            request.setAttribute("message", success ? "Specie updated successfully!" : "Failed to update specie.");
